        ...
        <div className="absolute w-full h-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3  gap-10 justify-center items-center px-8 md:px-12 lg:px-16   ">
          {cardarr.map((item) => (
            <div
              ...
              className="bg-white h-[15rem] md:h-[25rem] lg:w-[25rem] rounded-md  group overflow-hidden"
            >
              <img
                src={item.img.src}
                className=" lg:h-[18rem] lg:w-[25rem] md:w-[23rem] md:h-[15rem]  w-[24rem] h-[12rem] !transition-all group-hover:scale-105 ease-in-out "
              />
              <p className="md:text-2xl text-red-700 text-sm ">{item.title}</p>
              <span className=" flex gap-3">
                <img src={item.img1.src} className="h-3 w-6 md:h-6 md:w-14 " />{" "}
